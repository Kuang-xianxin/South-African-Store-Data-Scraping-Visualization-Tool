...
- 竞品“库存”必须标注为当前匿名会话、当前卖家和当前 SKU 的平台仓购物车可售上限，不得称为平台物理总库存；快捷菜单接受9件后必须优先解析购物车明确仓库库存提示，并在无明确值时通过自定义数量超量测试和二分校验尽力求精确上限。公开接口标记为 `is_leadtime` 的供应商调货/长时效到货商品必须统一标记“没货”，不计有效平台仓库存，也不得参与库存净流出销量推断。
...
- 竞品观察最小闭环已完成：支持多链接逐条采集、单条失败隔离、公开商品与完整评论、可选匿名购物车库存、历史快照、累计销量区间和观察期信号；该页面现为统一 Vue ERP 内的正式模块，与店铺经营页面共用 `8501` 和 SQLite。
- 首批4个竞品已写入本机 SQLite，共17条快照、192条去重评论；最新快照中2个供应商调货商品按规则显示“没货”，其余2个分别取得精确平台仓库存6和1。
