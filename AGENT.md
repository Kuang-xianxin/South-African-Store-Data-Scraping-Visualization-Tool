...
- 经营四象限必须固定标出横轴“近30天浏览量相对排名”和纵轴“近7日下单件数相对排名”；悬浮信息卡必须展示平台可售库存、指标截止日期、首次上架/最早记录、最近补货估算和近7日流量参考。
- Offers 接口的 `created_at` 是首次上架的优先来源；旧快照缺失时只能标为“本库最早记录”。平台没有补货事件接口时，最近补货只能按相邻有效库存记录上升估算。近7日流量参考只能按近30天日均浏览量乘7估算并显式标“估算”，不得称为真实7日流量或访客数。
...
- 经营四象限悬浮卡已加入平台库存、首次上架/本库最早记录、最近库存上升补货估算、近7日流量参考和指标截止日期，并在图框外固定显示横纵轴含义；MySQL Offer 当前表和历史快照表已增加可空 `created_at` 字段。
