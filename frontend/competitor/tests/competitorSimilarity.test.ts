...
test("every radar product-card surface exposes competitor query and the modal reuses the outer card", () => {
  ...
  assert.equal(pageSource.match(/@query-competitors="openCompetitorMatchModal"/g)?.length, 4);
  assert.equal(pageSource.match(/<CompetitorRadarProductCard/g)?.length, 4);
  ...
  assert.equal(pageSource.match(/<footer class="competitor-card-query-actions">/g)?.length, 1);
