...
useLiveUpdates("returns", async () => {
  await loadReturns(true); if (companySkuDetailOpen.value && companySkuDetailItem.value) await openCompanySkuDetail(companySkuDetailItem.value, undefined, true); return !error.value && (!companySkuDetailOpen.value || !companySkuDetailError.value);
}, {
  busy: () => loading.value || companySkuDetailLoading.value,
});
...
async function loadReturns(background: unknown = false) {
...
async function openCompanySkuDetail(item: SellerReturnItem, event?: Event, preserve = false) {
  const companySku = String(item.company_sku ?? "").trim();
  if (!companySku) return;
  if (!preserve) companySkuReturnFocusElement =
  ...
  companySkuDetailLoading.value = !preserve;
  try {
  ...
  <section class="returns-page">
    ...
    <template v-if="payload">
      ...
      <div v-else-if="payload.items.length" class="returns-table-wrap">
        <table class="returns-table mobile-record-table">
          <colgroup>
            <col class="returns-column-reference" />
            <col class="returns-column-product" />
            <col class="returns-column-store" />
            <col class="returns-column-reason" />
            <col class="returns-column-outcome" />
            <col class="returns-column-comment" />
            <col class="returns-column-transaction" />
          </colgroup>
          <thead>
            <tr>
              <th scope="col">退货日期 / 编号</th>
              <th scope="col">商品</th>
              <th scope="col">店铺 / 数量</th>
              <th scope="col">退货原因</th>
          ...
          <tbody>
            ...
            >
                ...
                <div class="returns-product-card">
                  ...
                  <span class="returns-product-copy">
                    ...
                    <span class="returns-product-action">
                      {{ hasCompanySku(item) ? "查看同款退货" : "未关联公司 SKU" }}
                    </span>
                  </span>
                </div>
              </td>
              ...
              <td data-label="退货原因" data-mobile-wide><strong>{{ item.return_reason_label }}</strong></td>
              <td data-label="处理结果" data-mobile-wide>
                <strong>{{ outcomeText(item) }}</strong>
                <small v-if="!item.outcome_statuses.length">暂无处理结果</small>
              </td>
              <td data-label="客户备注" data-mobile-wide class="returns-comment">{{ item.customer_comment || "未提供" }}</td>
              <td data-label="交易展开" data-mobile-wide>
...
.returns-table-wrap { overflow: hidden; border: 1px solid var(--line); border-radius: 14px; background: var(--panel); }
.returns-table { width: 100%; min-width: 0; table-layout: fixed; border-collapse: collapse; font-size: .72rem; }
.returns-column-reference { width: 15%; }
.returns-column-product { width: 30%; }
.returns-column-store { width: 9%; }
.returns-column-reason { width: 10%; }
.returns-column-outcome { width: 11%; }
.returns-column-comment { width: 15%; }
.returns-column-transaction { width: 10%; }
.returns-table th, .returns-table td { min-width: 0; padding: 12px; border-bottom: 1px solid var(--line); overflow-wrap: anywhere; text-align: left; vertical-align: top; }
...
.returns-product-card { display: flex; gap: 10px; min-width: 0; width: 100%; padding: 8px; border: 1px solid transparent; border-radius: 11px; background: transparent; color: inherit; text-align: left; cursor: inherit; }
...
.returns-comment { min-width: 0; max-width: none; overflow-wrap: anywhere; line-height: 1.5; }
...
@media (max-width: 760px) {
  .returns-page { min-width: 0; }
  .returns-summary-grid, .company-sku-summary { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  .returns-filters { grid-template-columns: minmax(0, 1fr); }
  .returns-filter-actions { display: grid; grid-template-columns: 1fr 1fr; }
  .returns-pagination { align-items: stretch; }
  .returns-pagination > div { display: flex; flex-wrap: wrap; gap: 8px; }
  .returns-product-card { grid-template-columns: 56px minmax(0, 1fr); gap: 10px; }
  .returns-product-media { width: 56px; height: 56px; }
  .company-sku-modal-header { align-items: flex-start; }
  .company-sku-modal-header h2 { overflow-wrap: anywhere; }
  .company-sku-modal-footer { padding-bottom: calc(13px + env(safe-area-inset-bottom)); }
}
