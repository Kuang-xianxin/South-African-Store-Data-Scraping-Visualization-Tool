...
def collect_offers(
    ...
    run_id = _persist_run_start(repository, "offers")
    try:
        ...
        with repository.transaction():
            repository.prune_offer_snapshot(
                sast_date(captured_at), [record.offer_id for record in records]
            )
            for record in records:
                repository.upsert_offer_snapshot(record, sast_date(captured_at))
...
def _persist_run_start(repository: Repository, run_type: str) -> str:
    with repository.transaction():
        return repository.begin_run(run_type)
