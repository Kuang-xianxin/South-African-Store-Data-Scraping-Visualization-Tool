...
Offers 接口提供 `created_at` 时，“首次上架”使用该平台时间；旧历史没有该字段时明确标为“本库最早记录”，不会伪装成平台真实上架时间。系统逐条比较同一商品的有效平台库存快照：后一条库存高于前一条时，后一条快照的实际采集时间就是“最近补货时间”，同时显示较前次增加数量；该时间按北京时间展示，不属于估算。没有观察到库存增加时显示暂无记录。接口只有近30天滚动浏览量，因此悬浮卡中的“近7日流量参考”固定按“近30天浏览量 ÷ 30 × 7”估算并明确标注，不代表真实近7日流量或访客数。
