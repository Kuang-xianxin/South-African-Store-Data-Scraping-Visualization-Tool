...
平台当前会拒绝原来的普通 HTTP 公开接口请求，因此竞品商品、变体和评论改由 Playwright 启动本机 Chrome/Edge，在同一个浏览器会话中逐个导航公开 JSON 地址；库存仍在另一个隔离匿名购物车会话中探测。采集器、ERP 接口和命令行入口均使用异步调用，批量采集时会复用会话而不是每条链接重复启动浏览器。
