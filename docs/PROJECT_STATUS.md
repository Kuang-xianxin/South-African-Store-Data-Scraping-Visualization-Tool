...
第 7、8 项的代码与合成数据验收已经完成；尚未使用真实店铺 API Key 或拉取真实店铺数据。
...
- 启动器重点测试：`4 passed`
- 完整测试：`123 passed`
- 总覆盖率：`90%`；API 客户端 `95%`、Repository `97%`、MetricService `97%`
- Ruff：通过
...
1. 用户在本机配置 Key 后，只做官方只读接口的真实联调。
2. 把真实联调结论补入审计记录，通过后再最终 commit 和 push。
