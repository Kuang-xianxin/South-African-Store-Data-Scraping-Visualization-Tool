...
1. 实施第 7 项：五个 CLI 命令、项目根目录 `.env` 自动加载、七天 SAST 刷新、质量检查、备份保留 8 份、可选计划任务安装脚本和中文运营说明。
2. 实施第 8 项：四种输出一致性、安全与脱敏、分页中断保护、安装包和最终测试。
3. 用户在本机配置 Key 后，只做官方只读接口的真实联调；通过后再最终 commit 和 push。
