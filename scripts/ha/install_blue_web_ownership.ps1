...
$BlueInstaller="$BlueRoot\staging\apply_blue_web_ownership.py"
...
try{
 ...
 $BlueReport | ConvertTo-Json | Set-Content -LiteralPath "$BlueRoot\state\web-ownership-install.json" -Encoding UTF8
}catch{
 Start-ScheduledTask -TaskName $BlueTaskName
 throw
}
